function GBaySideBarClosed(DFrame, tab, settingbtnclicked, data, firstjoined)
	if IsValid(SideBarOpened) then
		SideBarOpened:Remove()
	end

	hook.Call("GBaySideBarClosed")
	LocalPlayer().GBayOpenMenuTabStatus = false

	if IsValid(GBayDFrameLogo) then
		GBayDFrameLogo:Remove()
	end

	if IsValid(SideBarClosed) then
		SideBarClosed:Close()
	end
	...
	SideBarOpenBtn.Paint = function(s, w, h) end
...
function GBaySideBarOpened(DFrame, tab, settingbtnclicked, data, firstjoined)
	...
	elseif tab == "Settings" or settingbtnclicked then
		...
		if playerisadmin then
			...
			local configs = {{"Please enter your MySQL Info!", "bttn:Submit MySQL Data", [[function GBayConfigsButtonFunction() net.Start("GBaySetMySQLFromConfig") net.SendToServer() end]]}, {"Whats your server name?", "text:15:Server Name", servername}, {"Can people pay for ads?", "bool", adsyon, adsboolt, adsboolf}, {"Can people sell services?", "bool", servsyon, servboolt, servboolf}, {"Can people create coupons?", "bool", coupsyon, coupst, coupf}, {"What is the fee for posting items?", "numb:Posting Fee", ffpi}, {"What is the max price for items?", "numb:Max price", mpfpi}, {"Item tax (%)", "numb:Tax (for 8% just type 8)", itfpi}, {"Time to notify players of orders", "numb:Time in minutes to notify players", ttnoo}, {"Mail NPC pos", "text:255:100.0, 100.0, 100.0", npcposn}, {"Mail NPC ang", "text:255:100.0, 100.0, 100.0", npcangn}, {"Input Mail NPC's Model?", "text:255:NPC Model", npcmodels}}
	...
	elseif tab == "PlayerProfile" then
		...
		if LocalPlayer():GBayIsSuperAdmin(data) then
			...
			BanBTN.DoClick = function()
				Derma_StringRequest("GBay Ban Player", "How many days should this ban be?", "1", function(text)
			...
			SetRankBTN.DoClick = function()
				Derma_StringRequest("GBay Set Player Rank", "What rank should this player be? (Superadmin or Admin only!)", "Admin", function(text)
			...
			EditUserBTN.DoClick = function()
				...
				menu:AddOption("Edit +rep", function()
					Derma_StringRequest("GBay Set Player +rep", "What should the +rep be for this player?", "0", function(text)
				...
				menu:AddOption("Edit neutral rep", function()
					Derma_StringRequest("GBay Set Player neutral rep", "What should the neutral rep be for this player?", "0", function(text)
	...
	elseif tab == "CreateAds" then
		...
		PickItemToAd.OnSelect = function(panel, index, value)
			if isnumber(panel:GetOptionData(index)[1][5]) then
				theprice = panel:GetOptionData(index)[1][5]
			else
				theprice = panel:GetOptionData(index)[1][6]
			end
	...
	elseif tab == "EditShip" then
		...
		SelWepBtn.DoClick = function() end
		local SelWepPrice = vgui.Create("DNumSlider", SideBarOpened)
		SelWepPrice:SetPos(20, 430)
		SelWepPrice:SetSize(SideBarOpened:GetWide() - 40, 20)
		SelWepPrice:SetText("")
	...
	elseif tab == "EditEnt" then
		...
		SelEntBtn.DoClick = function() end
		local SelEntPrice = vgui.Create("DNumSlider", SideBarOpened)
		SelEntPrice:SetPos(20, 430)
		SelEntPrice:SetSize(SideBarOpened:GetWide() - 40, 20)
		SelEntPrice:SetText("")
	...
	elseif tab == "Purchase" then
		if LocalPlayer().GBayBuyingItemT == "Shipment" then
			...
			TheTaxRate:SetText("Price: " .. DarkRP.formatMoney(math.Round(totalprice)))
			...
			TheTaxRate:SetText("Tax Rate: " .. math.Round(GBayConfig.TaxToMultiplyBy * 100) .. "%")
			TheTaxRate:SetFont("GBayLabelFont")
			TheTaxRate:SetTextColor(Color(137, 137, 137, 255))
			TheTaxRate:SizeToContents()
			local TheTaxPrice = vgui.Create("DLabel", SideBarOpened)
			TheTaxPrice:SetPos(20, SideBarOpened:GetTall() - 120)
			TheTaxPrice:SetText("Tax: " .. DarkRP.formatMoney(math.Round(totalprice * GBayConfig.TaxToMultiplyBy)))
			...
			TheFinalPrice:SetText("Subtotal: " .. DarkRP.formatMoney(math.Round(totalprice)))
