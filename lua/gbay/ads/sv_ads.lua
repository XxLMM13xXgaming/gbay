        ...
        if ItemToAd[1].affected > 0 then
            if ItemToAd[1].data[1].sidmerchant == ply:SteamID64() and ply:getDarkRPVar("money") >= ItemToAd[1].data[1].price / 2 then
                ...
                GBayMySQL:Query("SELECT * FROM ads WHERE type='" .. querytype .. "' AND iid='" .. adid .. "'", function(adscheckexist)
                    if adscheckexist[1].status == false then print("GBay MySQL Error: " .. adscheckexist[1].error) elseif adscheckexist[1].affected == 0 then ply:GBayNotify("error", "This item is already advertised!") end
