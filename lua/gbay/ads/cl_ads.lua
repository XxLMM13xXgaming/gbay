function GBayAdsPageFull(DFrame, data)
  ...
  for k, v in pairs(data[6]) do
    if v[3] == "shipments" then table.insert(shipids,#shipids + 1,v[4]) elseif v[3] == "service" then table.insert(servids,#servids + 1,v[4]) elseif v[3] == "entities" then table.insert(entids,#entids + 1,v[4]) end
  end

  for k, v in pairs(data[3]) do
    if table.HasValue(shipids,v[1]) then
  ...
  for k, v in pairs(data[4]) do
    if table.HasValue(servids,v[1]) then
      local ItemMain = vgui.Create("DFrame")
  		ItemMain:SetSize( 270, 130 )
  		ItemMain:SetDraggable( false )
  		ItemMain:SetTitle( "" )
  ...
  for k, v in pairs(data[5]) do
    if table.HasValue(entids,v[1]) then
      local ItemMain = vgui.Create("DFrame")
  		ItemMain:SetSize( 270, 130 )
  		ItemMain:SetDraggable( false )
  		ItemMain:SetTitle( "" )
...
function GBayAdsPageSmall(DFrame, data)
  LocalPlayer().TabCurrentlyOn = "Ads"
  if IsValid(HomePanel) then HomePanel:Remove() end
	if IsValid(HomePanel2) then HomePanel2:Remove() end
	if !IsValid(DFrame) then return end
